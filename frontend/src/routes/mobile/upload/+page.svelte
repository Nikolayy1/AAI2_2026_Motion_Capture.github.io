<script lang="ts">
  ...
  async function loadPatients() {
    ...
    try {
      ...
      uploadError =
        "Failed to load patients. Please check if the backend server is running.";
      patients = [];
    } finally {
    ...
    <form onsubmit={handleContinue}>
      ...
      {#if previewSrc}
        <div class="preview-wrapper">
          ...
          <video src={previewSrc} bind:currentTime controls preload="metadata"
          ></video>
        </div>
      {/if}
