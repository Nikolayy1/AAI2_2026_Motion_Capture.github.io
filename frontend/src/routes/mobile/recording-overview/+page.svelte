<script lang="ts">
	...
	function uploadWithProgress(file: File, baseProgress: number): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			const biometrics = coerceBiometrics(
				uploadData.biometrics || getPatientBiometrics(patientId)
			);

			if (biometrics.age) formData.append('age', biometrics.age);
			if (biometrics.gender) formData.append('gender', biometrics.gender);
			if (biometrics.height) formData.append('height', biometrics.height);
			if (biometrics.weight) formData.append('weight', biometrics.weight);
			if (biometrics.notes) formData.append('notes', biometrics.notes);
