<script lang="ts">
  ...
  async function loadPatientData(id: number) {
    try {
      const endpoint = API_CONFIG.PATIENT_VIDEOS_ENDPOINT.replace(
        ":id",
        id.toString(),
      );
  ...
  async function updatePatient(biometricData: any) {
    ...
    try {
      ...
      const endpoint = API_CONFIG.UPDATE_PATIENT_ENDPOINT.replace(
        "{patient_id}",
        patientId.toString(),
      );
      const response = await apiFetch(endpoint, {
...
<div class="container">
  ...
  >
    ...
    <section class="form-section">
      ...
      <div class="form-group">
        <textarea bind:value={notes} placeholder="Notes (optional)" rows="4"
        ></textarea>
      </div>
    </section>

    <button type="submit" class="save-button" disabled={isCreatingPatient}
      >{buttonText()}</button
    >
  </form>
    ...
    <div class="success-overlay">
      <div class="success-modal">
        ...
        <h2 class="success-title">
          {mode === "edit" ? "Patient Updated!" : "Patient Created!"}
        </h2>
